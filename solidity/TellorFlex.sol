...
contract TellorFlex {
    ...
    function removeValue(bytes32 _queryId, uint256 _timestamp) external {
        ...
        require(_timestamp == _report.timestamps[_index], "invalid timestamp"); //is this line ever reached
    ...
    function _updateStakeAndPayRewards(
        ...
        if(rewardRate == 0) {
            rewardRate =
            (stakingRewardsBalance - ((accumulatedRewardPerShare * totalStakeAmount) / 1e18 - totalRewardDebt)) /
            30 days;
        }
    }
...
// ; (defun testing (empty lis)
// ;   (if (= false (last lis)) (+ empty (drop-last lis)) lis)
// ; )
// ;  [1 2 3 4 5 6 7 8 9]
// ; len (- (lenght list) 1)
// ; start/middle (/ len 2)
// ; left (- start 1)
// ; right (+ start 1)
// ; (if (and (= timestamp (at right list)) (at right is-disputed)) {'found: true, 'idx: start/middle}
// ;   (if (> timestamp (at right list) (if (not is-disputed) {'found: true, 'idx: start/middle} )))
//
// ; (defun test ()
// ; (fold (remove-ends-if-disputed) { 'lis: [true true true false true true false false] } (enumerate 0 (length [true true true false true true false false]))
// ;  ))
// ; short circuit
// ; (if (and (= timestamp (at right list)) (at right is-disputed)) {'found: true, 'idx: start/middle}
// if (_time >= _timestamp) return (false, 0);
// (if (>= (at 0 'timestamps) timestamp) "No values before this timestamp" )
// (if (and (and (> timestamp (at right lis)) (< timestamp (at left lsi)) (not is-disputed))))
// {'found: false, 'left: (/ (length lis) 2), 'right:(+ (/ (length lis) 2) 1), 'is-disputed: true}
